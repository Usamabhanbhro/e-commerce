...
| `OAUTH_SERVER_URL`       | Yes      | Existing OAuth provider base URL.                                                                  |
| `OAUTH_CLIENT_ID`        | Yes      | Existing OAuth client identifier.                                                                  |
| `OAUTH_CLIENT_SECRET`    | Yes      | Existing OAuth client secret.                                                                      |
| `OWNER_OPEN_ID`          | No       | Optional owner identity mapping used by the existing authorization implementation when applicable. |
...
The workflow fails before API startup if any required secret is missing or if either required production-like secret is shorter than 32 characters. It does not fabricate PostgreSQL, OAuth, R2, payment, or tunnel credentials. It does not replace PostgreSQL, Drizzle, Express, React, or the existing storage abstraction.

The workflow is classified as **STAGING PARTIALLY PROVISIONED** until a real run has completed with authorized R2, OAuth, and session/webhook secrets. It must never be used to claim production readiness. The intended successful classification after all runtime tests pass is **TEMPORARY STAGING OPERATIONAL**.
